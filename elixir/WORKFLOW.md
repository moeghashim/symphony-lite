...
The agent talks to Linear via the `linear_graphql` tool injected by Symphony's app-server. If it is not present, stop and ask the user to configure Linear. Do not use a Linear MCP server. Keep GraphQL calls narrow and prefer `sync_workpad` to update the persistent workpad comment from a local file.
...
- `land`: when ticket reaches `Merging`, use the `land` skill, which includes the `land` loop.
