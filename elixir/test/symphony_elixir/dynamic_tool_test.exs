defmodule SymphonyElixir.Codex.DynamicToolTest do
  ...
  defp write_tmp_workpad(content) do
    path = Path.join(System.tmp_dir!(), "test_workpad_#{System.unique_integer([:positive])}.md")
    File.write!(path, content)
    path
  end

  test "sync_workpad creates a comment from file when no comment_id is given" do
    test_pid = self()
  ...
  test "sync_workpad updates an existing comment when comment_id is given" do
    test_pid = self()
  ...
  test "sync_workpad validates required arguments before calling Linear" do
    ...
    assert Jason.decode!(no_issue["output"])["error"]["message"] =~ "issue_id"
    ...
    assert Jason.decode!(no_path["output"])["error"]["message"] =~ "file_path"
  end
  ...
  test "sync_workpad reports unreadable file paths" do
    response =
      DynamicTool.execute(
        ...
        %{
          "issue_id" => "ENG-42",
          "file_path" => "/tmp/does_not_exist_#{System.unique_integer([:positive])}.md"
        },
        linear_client: fn _query, _variables, _opts ->
    ...
    assert response["success"] == false
    assert Jason.decode!(response["output"])["error"]["message"] =~ "cannot read"
  end
end
